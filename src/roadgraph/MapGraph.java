...
public class MapGraph {
	...
	public void addEdge(GeographicPoint from, GeographicPoint to, String roadName, String roadType, double length)
		...
		if (!graph.containsKey(from) || !graph.containsKey(to)) throw new IllegalArgumentException();
		// check strings
		if (roadName == null || roadType == null) 
			throw new IllegalArgumentException();
		// check length
		if (length < 0) throw new IllegalArgumentException();
		// create a new edge from what we have been given
	...
	public List<GeographicPoint> bfs(GeographicPoint start, GeographicPoint goal,
		...
		LinkedList<GeographicPoint> queue = new LinkedList<GeographicPoint>(); // lifo
		...
		while (!queue.isEmpty()) {
			GeographicPoint curr = queue.pop();
			parent.addLast(curr);
			...
			for (Edge neighbor : graph.get(curr)) {
				...
				if (!visited.containsKey(neighborLocation)) {
					...
					queue.addFirst(neighborLocation);
				}
			}
		}
	...
	// print is a helper function that just prints out the graph for visualization and debugging purposes. 
	public void print() {
		graph.forEach((k, v) -> {
			System.out.print(k.x + "[");
			v.forEach(item -> {
				System.out.print(item.getRoadName() + " -> ");
			});
			System.out.println("]");
		});
